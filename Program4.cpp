...
public:
    ...
    CricketMatch(const CricketMatch &m) {
        for (int i = 0; i < 2; i++) {
            batsmanName[i] = m.batsmanName[i];
            batsmanRuns[i] = m.batsmanRuns[i];
            batsmanBalls[i] = m.batsmanBalls[i];
            batsmanOuts[i] = m.batsmanOuts[i];
            bowlerName[i] = m.bowlerName[i];
            runsConceded[i] = m.runsConceded[i];
            ballsBowled[i] = m.ballsBowled[i];
            wickets[i] = m.wickets[i];
            wides[i] = m.wides[i];
        }
        batsmanOutCount = m.batsmanOutCount;
        currentStriker = m.currentStriker;
        nonStriker = m.nonStriker;
        currentBowler = m.currentBowler;
    }
    ...
    void batsman_details() {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            cout << batsmanName[i] << ": " << batsmanRuns[i] << " (" << batsmanBalls[i] << ") - SR: " << sr << "\n";
